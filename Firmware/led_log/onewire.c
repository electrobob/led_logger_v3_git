...
XMEGA support by Bogdan Raducanu (bogdan@electrobob.com)
...
12/2013 - changed pin access to work on XMEGA, not everything is supported
*/
...
//configure the one wire pin for the sensors
#define OW_wire    IOPORT_CREATE_PIN(PORTC, 1)


#ifdef OW_ONE_BUS

#define OW_GET_IN() (ioport_get_pin_level(OW_wire))
#define OW_OUT_LOW()  (ioport_set_pin_level(OW_wire, 0))
#define OW_OUT_HIGH() (ioport_set_pin_level(OW_wire, 1))
#define OW_DIR_IN() (ioport_set_pin_dir(OW_wire, IOPORT_DIR_INPUT))
#define OW_DIR_OUT() (ioport_set_pin_dir(OW_wire, IOPORT_DIR_OUTPUT))
